...
玩家在每回合的戰鬥階段中，必須能選擇執行「普通攻擊」或「使用手牌（卡牌技能）」。普通攻擊不消耗 SP 且會使 SP 回復 2 點；使用手牌必須消耗該卡牌指定的 SP，若 SP 不足則不允許出牌。不管是普通攻擊還是使用卡牌，每次行動後玩家都必須回復 2 點 SP。最大 SP 上限必須設定為 15。

#### Scenario: Normal Attack SP Recovery
- **WHEN** 玩家選擇普通攻擊並確認執行
- **THEN** 玩家的 SP 必須增加 2 點（不超過最大能量上限 15），並對怪物造成基礎攻擊傷害

#### Scenario: Card Usage SP Recovery
- **WHEN** 玩家成功打出手牌行動結束時
- **THEN** 玩家的 SP 必須增加 2 點（不超過最大能量上限 15）
